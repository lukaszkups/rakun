...
const saveSettings = async () => {
  ...
  if (
    localCanvasWidth.value !== store.state.canvasWidth ||
    localCanvasHeight.value !== store.state.canvasHeight ||
    localZoom.value !== store.state.zoom
  ) {
    ...
    const canvasClone = await cloneCanvasElement(
      store.state.canvasImageCtx.canvas,
    );
    // populate new project settings
    store.dispatch('updateProp', {
      name: 'canvasWidth',
      value: localCanvasWidth.value,
    });
    store.dispatch('updateProp', {
      name: 'canvasHeight',
      value: localCanvasHeight.value,
    });
    ...
    if (previousZoom !== store.state.zoom) {
      ...
      await loadAndResizeImageToCanvas(
        canvasClone,
        store.state.canvasImageCtx,
      ...
      await loadImageToCanvas(
        store.state.canvasImageCtx,
        imgData,
        localCanvasWidth.value * store.state.zoom,
        localCanvasHeight.value * store.state.zoom,
      );
      // redraw grid canvas
      await clearCanvas(store.state.canvasGridCtx, canvasWidth, canvasHeight);
      await drawGrid(
      ...
      await loadAndResizeImageToCanvas(
        canvasClone,
        store.state.canvasThumbnailCtx,
        localCanvasWidth.value,
        localCanvasHeight.value,
        previousZoom,
        2,
...
<template>
  <div class="rkn-project-settings rkn-form">
    ...
    <div class="rkn-field">
      ...
      <input v-model="localFps" name="fps" class="rkn-input" type="number" />
    </div>
    <div class="rkn-field">
    ...
    <button class="rkn-button" @click="saveSettings">Apply</button>
  </div>
</template>
